// Modyfikacja AddBranch względem wersji HTT
...
void MLAnalyzer::addBranch(TTree *tree)
{
	...
	if(!MLTree_)
	{
		...
		if(tree)
		{
			...
			tree->Branch("pt_1", &pt_1_);
			tree->Branch("pt_2", &pt_2_);
			tree->Branch("eta_1", &eta_1_);
			tree->Branch("eta_2", &eta_2_);
			tree->Branch("phi_1", &phi_1_);
			tree->Branch("phi_2", &phi_2_);
			tree->Branch("ibeta_1", &ibeta_1_);
			tree->Branch("ibeta_2", &ibeta_2_);
			tree->Branch("err_ibeta_1", &err_ibeta_1_);
			tree->Branch("err_ibeta_2", &err_ibeta_2_);
			tree->Branch("n_CSC_hits_1", &n_CSC_hits_1_);
			tree->Branch("n_CSC_hits_2", &n_CSC_hits_2_);
			tree->Branch("n_DT_hits_1", &n_DT_hits_1_);
			tree->Branch("n_DT_hits_2", &n_DT_hits_2_);
			tree->Branch("n_RPC_hits_1", &n_RPC_hits_1_);
			tree->Branch("n_RPC_hits_2", &n_RPC_hits_2_);
			tree->Branch("n_strip_hits_1", &n_strip_hits_1_);
			tree->Branch("n_strip_hits_2", &n_strip_hits_2_);
			tree->Branch("n_pixel_hits_1", &n_pixel_hits_1_);
			tree->Branch("n_pixel_hits_2", &n_pixel_hits_2_);
			tree->Branch("n_TOF_meas_1", &n_TOF_meas_1_);
			tree->Branch("n_TOF_meas_2", &n_TOF_meas_2_);
			tree->Branch("I_h_1", &I_h_1_);
			tree->Branch("I_h_2", &I_h_2_);
			tree->Branch("I_as_1", &I_as_1_);
			tree->Branch("I_as_2", &I_as_2_);
			tree->Branch("mass_I_h_1", &mass_I_h_1_);
			tree->Branch("mass_I_h_2", &mass_I_h_2_);
			tree->Branch("mass_ibeta_1", &mass_ibeta_1_);
			tree->Branch("mass_ibeta_2", &mass_ibeta_2_);
			tree->Branch("pt_err_1", &pt_err_1_);
			tree->Branch("pt_err_2", &pt_err_2_);
			tree->Branch("lumi_sec", &lumi_sec_);
			tree->Branch("event_nr", &event_nr_);
			tree->Branch("run_nr", &run_nr_);
			tree->Branch("quality_index_1", &quality_index_1_);
			tree->Branch("quality_index_2", &quality_index_2_);
		}
		else
